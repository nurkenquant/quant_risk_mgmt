...
include("../lib/return_calculate.jl")
...
call_vals = [v for v in gbsm.(true, currentPrice, strike, ttm, rf, rf-dy, iv)]
put_vals  = [v for v in gbsm.(false, currentPrice, strike, ttm, rf, rf-dy, iv)]
...
dy = 0.0057
currentPrice = 170.15
options = CSV.read("./Week06/AAPL_Options.csv",DataFrame)
...
options[!,:ivol] = iv
options[!,:gbsm] = [v for v in gbsm.(options.Type.=="Call",currentPrice,options.Strike,options.ttm,rf,rf-dy,options.ivol)]
...
currentS=170.15
returns = return_calculate(CSV.read("Week06/DailyPrices.csv",DataFrame)[!,[:Date,:AAPL]],method="LOG",dateColumn="Date")[!,:AAPL]
returns = returns .- mean(returns)
sd = std(returns)
current_dt = Date(2023,10,30)

portfolio = CSV.read("Week06/problem3.csv", DataFrame)
